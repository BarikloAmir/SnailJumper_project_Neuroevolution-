...
def get_input_vector(player_x, obs, screen_width, screen_height, number):
    player_x_normal = player_x / screen_width
    x1=0
    x2=0
    x3=0
    y1=0
    ...
    bug = 0
    if number >= 1:
        x1 = obs[0].get('x')/screen_width
        y1 = obs[0].get('y')/screen_height
        #if 0.3<=x1<=0.67:
        #    bug = math.sqrt(x1*x1 +y1*y1)
    if number>=2:
        x2 = obs[1].get('x') / screen_width
        y2 = obs[1].get('y')/screen_height
        #if 0.3<=x2<=0.67:
        #    bug = math.sqrt(x1*x1 +y1*y1)
    if number>=3:
        x3 = obs[2].get('x') / screen_width
        y3 = obs[2].get('y')/screen_height
        #if 0.3<=x3<=0.67:
        #    bug = math.sqrt(x1*x1 +y1*y1)
    number = number/100

    return np.array([player_x_normal,x1,x2,x3,y1,y2,y3]).reshape(7,1)
...
class Player(pygame.sprite.Sprite):
    def __init__(self, game_mode):
        ...
        if self.game_mode == "Neuroevolution":
            ...
            layer_sizes = [7, 15, 2]  # TODO (Design your architecture here by changing the values)
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        input_vector = get_input_vector(player_x,obstacles, screen_width, screen_height,len(obstacles))
        #input_vector = np.reshape(np.array([0,0,0,0,0,0,0]),(7,1))
        #print(input_vector)
        output_vector = self.nn.forward(input_vector)


        if output_vector[0]>output_vector[1]:
